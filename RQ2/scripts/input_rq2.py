...
def borderline_cl(tests, num_tests, locs, f, method_map):
    table_wnoi = []
    lst = []
    for r in range(0, num_tests):
        row = [True] + [False] * locs
    ...
    # danger_lst = borderline(ob, label, 3)
    if (false_count < 10):
        return ob, label
    # Cross validation
...
def borderline_aug_cl(tests, num_tests, locs, f, method_map):
    lst = []
...
def aug_cl(tests, num_tests, locs, f, method_map):
    lst = []
    ob, label = only_aug(tests, num_tests, locs, f, method_map)
    ...
    labels = np.delete(label, ordered_lst)
    return charas, labels
def only_aug(tests, num_tests, locs, bin_file, method_map):
    table_wnoi = []
    ...
    ob_init = np.array(table_wnoi)
    label_init = np.array(tests)
    X_res, y_res = aug_data(ob_init, label_init, 3)
    return X_res, y_res

def only_cl(tests, num_tests, locs, f, method_map):
    table_wnoi = []
    lst = []
    for r in range(0, num_tests):
        row = [True] + [False] * locs
...
def fill_table(tests, num_tests, locs, f, method_map):
    ...
    # (borderline_aug_cl, borderline_aug, borderline_cl, aug_cl, only_aug and only_cl)
    ttable, tlabel = aug_cl(tests, num_tests, locs, f, method_map)
