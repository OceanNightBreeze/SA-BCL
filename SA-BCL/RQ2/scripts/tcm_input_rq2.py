...
def bordrline_cl(tests, num_tests, locs, f, method_map):
    table_wnoi = []
    lst = []
    for r in range(0, num_tests):
        row = [True] + [False] * locs
    ...
    false_count = np.sum(label_init == False)
    if (false_count < 10):
        return ob_init, label_init
    # Cross validation
    psx = np.zeros((len(label_init), 2))
    kf = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
    for train_index, test_index in kf.split(ob_init, label_init):
        X_train, X_test = ob_init[train_index], ob_init[test_index]
        Y_train, Y_test = label_init[train_index], label_init[test_index]
    ...
    charas = np.delete(ob_init, ordered_lst, axis=0)
    labels = np.delete(label_init, ordered_lst)
...
def only_aug(tests, num_tests, locs, f, method_map):
    table_wnoi = []
    for r in range(0, num_tests):
        row = [True] + [False] * locs
        # Construct the table row
...
def fill_table(tests, num_tests, locs, f, method_map):
    ...
    ttable, tlabel = borderline_aug(tests, num_tests, locs, f, method_map)
